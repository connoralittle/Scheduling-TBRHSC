...
def forbid_seq_continue_pattern(model, span, start, pred, prior):
    ...
    model.AddBoolOr([span_bool, prior.shifts[start + 1]]).OnlyEnforceIf(pred)
    ...
    for length in window_length(hard_max, grow_pred):
        ...
        for start in shift_window(shifts, length, prior, post):
            ...
            # If you can continue the pattern then either continue it or ban it
                ...
                if start == window_size(shifts, length, prior, post) - 1:
                    continue
    ...
    for length in range(hard_min, soft_min+1):
        for start in shift_window(shifts, length, prior, post):
            ...
            if continue_pattern:
               if start == window_size(shifts, length, prior, post) - 1:
                    continue
               or_vars = or_vars + [prior.shifts[start + 1]]
            if not prior_exists and not post_exists:
            ...
            cost_coefficients.append(min_cost * (length - hard_min))
